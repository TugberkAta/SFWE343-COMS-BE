...
const postApproveUser = async (req, res) => {
  ...
  const { userId, userTypeId, approvedStatus } = req.body;

  if (currentUserId === userId) {
    return res
      .status(400)
      .send({ message: "You cannot update your own approval or type." });
  }

  try {
    await postApproveUserSchema.validate(
      { userId, userTypeId, approvedStatus },
      { abortEarly: false }
    );

    await approveUser({ userId, userTypeId, approvedStatus });
