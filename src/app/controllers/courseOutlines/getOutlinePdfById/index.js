...
const buildOutlinePdfBuffer = require("~root/utils/outlinePdf/buildOutlinePdfBuffer");
...
const createFileName = outline => {
  ...
  return `${outline.courseCode}-${academicYear}-${semester}-outline.pdf`;
};

const getOutlinePdfById = async (req, res) => {
  ...
  try {
    ...
    const pdfBuffer = await buildOutlinePdfBuffer(outline);
...
module.exports = getOutlinePdfById;
